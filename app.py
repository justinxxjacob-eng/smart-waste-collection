...
def is_valid_email(email):
    ...
    if re.search(r'[bcdfghjklmnpqrstvwxyz]{6,}', local.lower()):
        return False
    # Reject if local part is ONLY numbers (e.g., 12345@gmail.com)
    if local.isdigit():
        return False
    # Reject if local part has less than 3 letters (e.g., ab@gmail.com, xy123@gmail.com ok ra)
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        zones = [
            ('Mahogany', 'Mahogany street and adjacent areas'),
            ('San Francisco', 'San Francisco residential zone'),
            ('Gmelina', 'Gmelina neighborhood area'),
            ('Arbor', 'Arbor residential community'),
            ('Narra', 'Narra avenue residential area'),
            ('Lemon', 'Lemon street and surrounding blocks'),
            ('Dunggon', 'Dunggon residential zone'),
        ]
    ...
    if c.fetchone()[0] == 0:
        ...
        c.execute("INSERT INTO households (user_id,address,barangay_zone,latitude,longitude) VALUES (?,?,?,?,?)",
                  (uid,'123 Mahogany St, Mahogany, Magsaysay, Zamboanga Sibugay','Mahogany',7.0707,125.6087))

        demo_residents = [
            ('Pedro Reyes','pedro@email.com','San Francisco','456 Mabini St, San Francisco, Magsaysay, Zamboanga Sibugay'),
            ('Ana Flores','ana@email.com','Gmelina','789 Aguinaldo Ave, Gmelina, Magsaysay, Zamboanga Sibugay'),
            ('Carlos Bautista','carlos@email.com','Mahogany','12 Bonifacio St, Mahogany, Magsaysay, Zamboanga Sibugay'),
            ('Lita Gomez','lita@email.com','Arbor','34 Rizal Ext, Arbor, Magsaysay, Zamboanga Sibugay'),
            ('Ramon Cruz','ramon@email.com','Mahogany','56 Del Pilar St, Mahogany, Magsaysay, Zamboanga Sibugay'),
            ('Sofia Garcia','sofia@email.com','San Francisco','78 Quezon Ave, San Francisco, Magsaysay, Zamboanga Sibugay'),
            ('Miguel Tan','miguel@email.com','Gmelina','90 Mabini Blvd, Gmelina, Magsaysay, Zamboanga Sibugay'),
            ('Elena Reyes','elena@email.com','Arbor','111 Rizal Ave, Arbor, Magsaysay, Zamboanga Sibugay'),
        ]
    ...
    # Seed schedules for 7 zones
    c.execute("SELECT COUNT(*) FROM collection_schedules")
    if c.fetchone()[0] == 0:
        days = ['Monday','Wednesday','Friday','Tuesday','Thursday','Monday','Tuesday']
        times = ['07:00','08:00','07:30','09:00','08:30','06:30','07:00']
    ...
    if c.fetchone()[0] == 0:
        ...
        for issue, desc, stat in [('missed pickup','Garbage truck did not come on Monday.','open'),('overflow','Bin overflowing near Gmelina market.','resolved'),('wrong schedule','Was told Wednesday but truck came Tuesday.','resolved')]:
    ...
    if request.method == 'POST':
        ...
        if address:
            ...
            if 'magsaysay' not in addr_lower:
                errors.append('Address must include "Magsaysay" (Barangay Magsaysay residents only).')
            if 'zamboanga' not in addr_lower:
                errors.append('Address must include "Zamboanga" (Zamboanga Sibugay area only).')
            if zone:
                zone_lower = zone.lower()
                zone_keywords = re.sub(r'[^a-z0-9]', ' ', zone_lower).split()
                matches = sum(1 for kw in zone_keywords if len(kw) > 1 and kw in addr_lower)
                zone_full_in_addr = zone_lower in addr_lower
                if not zone_full_in_addr and matches < 2:
                    errors.append(f'Your address must include "{zone}". Make sure your address matches your selected purok.')
        else:
            errors.append('Home address is required.')
        
        if not zone:
            errors.append('Please select your purok.')
...
def admin_dashboard():
    ...
    for z in zones:
        residents = conn.execute("""
            SELECT u.user_id, u.name, u.contact_number, h.address, h.household_id
            FROM users u
        ...
        if residents:
            ...
            for r in residents:
                last_log = conn.execute("""
                    SELECT status, collected_at, bin_count, bin_type, fill_level
                ...
                if last_log:
                    ...
                    last_collected = last_log['collected_at']
                    bin_count = last_log['bin_count'] or 0
                    bin_type = last_log['bin_type'] or ''
                    fill_level = last_log['fill_level'] or ''
                    if bin_count > 0:
                        bw = {'small_bag':8,'medium_bag':15,'large_bag':25,'small_drum':30,'medium_drum':60,'large_drum':100,'small_bin':20,'large_bin':40}
                        fm = {'quarter':0.25,'half':0.5,'mostly':0.75,'full':1.0,'overflow':1.3}
                        waste_volume = round(bin_count * bw.get(bin_type, 15) * fm.get(fill_level, 1.0), 1)
                    else:
                        waste_volume = 0
                else:
                    last_status = '— No data'
    ...
    all_residents = conn.execute("""
        SELECT u.user_id, u.name, h.address
        FROM users u
        JOIN households h ON u.user_id = h.user_id
    ...
    return render_template_string(ADMIN_HTML, total_households=th, total_users=tu, today_schedules=ts,
                                  ...
                                  zone_residents=zone_residents, all_residents=all_residents)
...
def collector_dashboard():
    ...
    assigned = conn.execute("""
        SELECT cs.*, z.zone_name, z.description, z.zone_id 
        FROM collection_schedules cs 
        JOIN zones z ON cs.zone_id = z.zone_id 
    ...
    rl = conn.execute("""
        ...
        ORDER BY cl.collected_at DESC LIMIT 20
    """, (cid,)).fetchall()
    
    st = conn.execute("""
        SELECT status, COUNT(*) as cnt 
        FROM collection_logs 
        WHERE collector_id=? AND date(collected_at)=date('now') 
        GROUP BY status
    """, (cid,)).fetchall()
...
def resident_dashboard():
    ...
    if hh:
        ...
        """, (uid,)).fetchall()
        ll = conn.execute("SELECT * FROM collection_logs WHERE user_id=? ORDER BY collected_at DESC LIMIT 1", (uid,)).fetchone()
    else:
    ...
    return render_template_string(RESIDENT_HTML, household=hh, schedule=schedule, 
                                  recent_collections=recent_collections, last_log=ll, 
                                  notifs=notifs, my_reports=mr, today=today, 
                                  format_time_ampm=format_time_ampm)
...
def api_send_notification():
    ...
    if recipient_ids and len(recipient_ids) > 0:
        for uid in recipient_ids:
            conn.execute("INSERT INTO notifications (user_id,message,type,status,sent_at) VALUES (?,?,?,?,?)", 
                        (uid, message, notif_type, 'sent', datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            sent_count += 1
    else:
        users = conn.execute("SELECT user_id FROM users WHERE role='resident'").fetchall()
        for u in users:
            conn.execute("INSERT INTO notifications (user_id,message,type,status,sent_at) VALUES (?,?,?,?,?)", 
                        (u['user_id'], message, notif_type, 'sent', datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            sent_count += 1
    
    conn.commit()
    conn.close()
    return jsonify({'status':'ok','sent':sent_count})
...
<head>
  ...
  <title>EcoTrack - Smart Barangay Waste Collection System</title>
  ...
  <style>
    :root {
      ...
      --green-800: #166534;--green-900: #14532d;--text: #0f172a;--text-muted: #64748b;
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; padding: 0 6vw; height: 68px; display: flex; align-items: center; justify-content: space-between; background: rgba(255,255,255,0.92); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(22,163,74,0.12); transition: box-shadow 0.3s; }
    nav.scrolled { box-shadow: 0 2px 20px rgba(0,0,0,0.07); }
    .nav-brand { display: flex; align-items: center; gap: 10px; text-decoration: none; }
    .nav-logo-icon { width: 36px; height: 36px; background: linear-gradient(135deg, var(--green-500), var(--green-700)); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 17px; box-shadow: 0 2px 10px rgba(34,197,94,0.3); }
    .nav-brand-text { font-size: 18px; font-weight: 800; color: var(--text); letter-spacing: -0.3px; }
    .nav-brand-text span { color: var(--green-600); }
    .nav-cta { display: inline-flex; align-items: center; gap: 6px; padding: 9px 22px; background: var(--green-600); color: #fff; border-radius: 8px; font-size: 13.5px; font-weight: 700; text-decoration: none; transition: all 0.18s; box-shadow: 0 2px 12px rgba(22,163,74,0.3); }
    .nav-cta:hover { background: var(--green-700); transform: translateY(-1px); box-shadow: 0 4px 18px rgba(22,163,74,0.4); }
    .hero { min-height: 100vh; display: grid; grid-template-columns: 1fr 1fr; align-items: center; padding: 100px 6vw 60px; gap: 60px; position: relative; overflow: hidden; }
    .hero-bg { position: absolute; inset: 0; background: radial-gradient(ellipse 60% 50% at 70% 50%, rgba(187,247,208,0.35) 0%, transparent 70%), radial-gradient(ellipse 40% 60% at 10% 80%, rgba(240,253,244,0.8) 0%, transparent 60%); pointer-events: none; }
    .hero-left { position: relative; z-index: 2; }
    .hero-badge { display: inline-flex; align-items: center; gap: 7px; background: var(--green-50); border: 1px solid var(--green-200); color: var(--green-700); font-size: 12px; font-weight: 700; padding: 6px 14px; border-radius: 20px; margin-bottom: 28px; text-transform: uppercase; letter-spacing: 0.06em; animation: fadeUp 0.6s ease both; }
    .hero-badge-dot { width: 7px; height: 7px; background: var(--green-500); border-radius: 50%; animation: pulse 2s infinite; }
    @keyframes pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.6; transform: scale(0.85); } }
    .hero-title { font-size: clamp(36px, 4.5vw, 58px); font-weight: 900; line-height: 1.08; letter-spacing: -1.5px; color: var(--text); margin-bottom: 24px; animation: fadeUp 0.6s 0.1s ease both; }
    .hero-title em { font-style: normal; color: var(--green-600); position: relative; display: inline-block; }
    .hero-title em::after { content: ''; position: absolute; left: 0; right: 0; bottom: -3px; height: 4px; background: linear-gradient(90deg, var(--green-400), var(--green-600)); border-radius: 2px; }
    .hero-desc { font-size: 17px; color: var(--text-muted); line-height: 1.75; max-width: 520px; margin-bottom: 40px; font-weight: 400; animation: fadeUp 0.6s 0.2s ease both; }
    .hero-actions { display: flex; align-items: center; gap: 16px; flex-wrap: wrap; animation: fadeUp 0.6s 0.3s ease both; }
    .btn-primary-hero { display: inline-flex; align-items: center; gap: 8px; padding: 14px 30px; background: linear-gradient(135deg, var(--green-500), var(--green-700)); color: #fff; border-radius: 10px; font-size: 15px; font-weight: 700; text-decoration: none; transition: all 0.2s; box-shadow: 0 4px 20px rgba(34,197,94,0.35); font-family: 'Plus Jakarta Sans', sans-serif; border: none; cursor: pointer; }
    .btn-primary-hero:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(34,197,94,0.45); }
    .btn-secondary-hero { display: inline-flex; align-items: center; gap: 8px; padding: 14px 26px; background: transparent; color: var(--green-700); border: 1.5px solid var(--green-300); border-radius: 10px; font-size: 15px; font-weight: 600; text-decoration: none; transition: all 0.2s; }
    .btn-secondary-hero:hover { background: var(--green-50); border-color: var(--green-500); }
    .hero-stats { display: flex; gap: 32px; margin-top: 52px; padding-top: 32px; border-top: 1px solid var(--border); animation: fadeUp 0.6s 0.4s ease both; }
    .hero-stat-value { font-size: 26px; font-weight: 800; color: var(--text); font-family: 'DM Mono', monospace; letter-spacing: -0.5px; }
    ...
    .hero-illustration { font-size: 180px; opacity: 0.9; animation: float 4s ease-in-out infinite; }
    @keyframes float { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
    .features { padding: 100px 6vw; background: var(--green-50); }
    .section-label { text-align: center; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--green-600); margin-bottom: 14px; }
    .section-title { text-align: center; font-size: clamp(28px, 3vw, 40px); font-weight: 800; letter-spacing: -0.8px; color: var(--text); margin-bottom: 14px; }
    .section-desc { text-align: center; font-size: 16px; color: var(--text-muted); max-width: 520px; margin: 0 auto 64px; line-height: 1.7; }
    ...
    .feature-card { background: #fff; border: 1px solid var(--border); border-radius: 16px; padding: 28px; transition: all 0.22s; position: relative; overflow: hidden; }
    .feature-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, var(--green-400), var(--green-600)); transform: scaleX(0); transform-origin: left; transition: transform 0.3s; }
    .feature-card:hover { transform: translateY(-4px); box-shadow: 0 12px 32px rgba(0,0,0,0.08); border-color: var(--green-200); }
    .feature-card:hover::before { transform: scaleX(1); }
    .feature-icon { width: 50px; height: 50px; background: var(--green-50); border: 1px solid var(--green-200); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 22px; margin-bottom: 18px; }
    ...
    .roles { padding: 100px 6vw; background: #fff; }
    .roles-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; max-width: 1000px; margin: 0 auto; }
    .role-card { border-radius: 16px; padding: 32px; text-align: center; transition: transform 0.2s; }
    .role-card:hover { transform: translateY(-4px); }
    .role-card.admin { background: linear-gradient(145deg, var(--green-800), var(--green-900)); color: #fff; }
    .role-card.collector { background: linear-gradient(145deg, var(--green-600), var(--green-700)); color: #fff; }
    .role-card.resident { background: var(--green-50); border: 1.5px solid var(--green-200); color: var(--green-900); }
    .role-emoji { font-size: 36px; margin-bottom: 16px; display: block; }
    .role-title { font-size: 18px; font-weight: 800; margin-bottom: 10px; }
    .role-desc { font-size: 13px; line-height: 1.7; opacity: 0.85; }
    .role-features { margin-top: 18px; text-align: left; display: flex; flex-direction: column; gap: 8px; }
    .role-feature { display: flex; align-items: center; gap: 8px; font-size: 12.5px; font-weight: 500; }
    .role-feature-check { width: 16px; height: 16px; border-radius: 50%; background: rgba(255,255,255,0.25); display: flex; align-items: center; justify-content: center; font-size: 10px; flex-shrink: 0; }
    .role-card.resident .role-feature-check { background: var(--green-200); }
    .cta-section { padding: 80px 6vw; background: linear-gradient(135deg, var(--green-700), var(--green-900)); text-align: center; position: relative; overflow: hidden; }
    .cta-section::before { content: ''; position: absolute; top: -60px; right: -60px; width: 300px; height: 300px; border-radius: 50%; background: rgba(255,255,255,0.04); }
    .cta-section::after { content: ''; position: absolute; bottom: -80px; left: -40px; width: 250px; height: 250px; border-radius: 50%; background: rgba(255,255,255,0.04); }
    .cta-label { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--green-300); margin-bottom: 16px; }
    .cta-title { font-size: clamp(28px, 3vw, 44px); font-weight: 900; color: #fff; letter-spacing: -1px; margin-bottom: 18px; }
    .cta-desc { font-size: 16px; color: rgba(255,255,255,0.75); max-width: 480px; margin: 0 auto 36px; line-height: 1.7; }
    .btn-cta-white { display: inline-flex; align-items: center; gap: 8px; padding: 15px 36px; background: #fff; color: var(--green-700); border-radius: 10px; font-size: 15px; font-weight: 800; text-decoration: none; transition: all 0.2s; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }
    .btn-cta-white:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(0,0,0,0.2); }
    footer { background: var(--text); color: rgba(255,255,255,0.6); text-align: center; padding: 28px 6vw; font-size: 13px; }
...
<body>
  <nav id="navbar">
    <a href="/" class="nav-brand">
      ...
      <span class="nav-brand-text">Eco<span>Track</span></span>
    </a>
  ...
  <section class="hero">
    ...
    <div class="hero-left">
      <div class="hero-badge">
        ...
        Barangay Magsaysay, Zamboanga Sibugay
      </div>
      <h1 class="hero-title">
        Smarter Waste.<br>
        Cleaner <em>Barangay</em>.<br>
        Better Future.
      </h1>
      <p class="hero-desc">
        EcoTrack uses machine learning to predict waste levels, schedule pickups intelligently, and keep every household in your barangay informed — in real time.
      </p>
      <div class="hero-actions">
        <a href="/login" class="btn-primary-hero">Get Started →</a>
        <a href="#features" class="btn-secondary-hero">Learn More ↓</a>
      </div>
      <div class="hero-stats">
        <div>
          <div class="hero-stat-value">7</div>
          <div class="hero-stat-label">Puroks</div>
        </div>
        <div>
        ...
        <div>
          <div class="hero-stat-value">ML</div>
          <div class="hero-stat-label">Powered Predictions</div>
        </div>
      </div>
    </div>
  ...
  <section class="features" id="features">
    <p class="section-label reveal">Key Features</p>
    <h2 class="section-title reveal">Everything Your Barangay Needs</h2>
    <p class="section-desc reveal">From ML-powered predictions to real-time notifications — one platform for residents, collectors, and administrators.</p>
    <div class="features-grid">
      <div class="feature-card reveal"><div class="feature-icon">🤖</div><div class="feature-title">ML-Powered Predictions</div><div class="feature-desc">Machine learning analyzes historical waste data to forecast overflow risk up to 14 days ahead — so you're always one step ahead.</div></div>
      <div class="feature-card reveal"><div class="feature-icon">🗺️</div><div class="feature-title">Purok-Based Monitoring</div><div class="feature-desc">7 barangay puroks tracked in real time. See which areas are collected, missed, or at risk — all from one dashboard.</div></div>
      <div class="feature-card reveal"><div class="feature-icon">🔔</div><div class="feature-title">Smart Notifications</div><div class="feature-desc">Admins can send targeted alerts to specific residents. Residents receive pickup schedules and updates directly.</div></div>
      <div class="feature-card reveal"><div class="feature-icon">📊</div><div class="feature-title">Analytics Dashboard</div><div class="feature-desc">Visualize waste volume trends, collection efficiency, and peak days. Data-driven decisions for better barangay planning.</div></div>
      <div class="feature-card reveal"><div class="feature-icon">🚛</div><div class="feature-title">Collector Route Logging</div><div class="feature-desc">Collectors log each pickup with bin count, type, and fill level. Estimated waste volume calculated automatically per household.</div></div>
      <div class="feature-card reveal"><div class="feature-icon">📋</div><div class="feature-title">Resident Reports</div><div class="feature-desc">Residents can report missed pickups, overflowing bins, or wrong schedules. Admins resolve issues from one centralized view.</div></div>
    </div>
  </section>

  <section class="roles" id="roles">
    <p class="section-label reveal">Who It's For</p>
    <h2 class="section-title reveal">Built for Every Role</h2>
    <p class="section-desc reveal">Three portals, one system — designed specifically for how each person interacts with waste collection.</p>
    <div class="roles-grid">
      <div class="role-card admin reveal">
        <span class="role-emoji">🧑‍💼</span>
        <div class="role-title">Admin</div>
        <div class="role-desc">Full control over puroks, schedules, users, and the ML engine.</div>
        <div class="role-features">
          <div class="role-feature"><div class="role-feature-check">✓</div> Run ML predictions</div>
          <div class="role-feature"><div class="role-feature-check">✓</div> Manage puroks & schedules</div>
          <div class="role-feature"><div class="role-feature-check">✓</div> Send targeted alerts</div>
          <div class="role-feature"><div class="role-feature-check">✓</div> Broadcast notifications</div>
        </div>
      </div>
      <div class="role-card collector reveal">
        <span class="role-emoji">🚛</span>
        <div class="role-title">Collector</div>
        <div class="role-desc">Log each pickup with waste details and view ML purok forecasts.</div>
        <div class="role-features">
          <div class="role-feature"><div class="role-feature-check">✓</div> View assigned routes</div>
          <div class="role-feature"><div class="role-feature-check">✓</div> Log per-household status</div>
          <div class="role-feature"><div class="role-feature-check">✓</div> Live waste estimator</div>
          <div class="role-feature"><div class="role-feature-check">✓</div> See ML predictions</div>
        </div>
      </div>
      <div class="role-card resident reveal">
        <span class="role-emoji">🏡</span>
        <div class="role-title">Resident</div>
        <div class="role-desc">Track your household's pickups, schedules, and submit reports.</div>
        <div class="role-features">
          <div class="role-feature"><div class="role-feature-check" style="background:var(--green-300);">✓</div> View collection schedule</div>
          <div class="role-feature"><div class="role-feature-check" style="background:var(--green-300);">✓</div> Collection history</div>
          <div class="role-feature"><div class="role-feature-check" style="background:var(--green-300);">✓</div> Receive notifications</div>
          <div class="role-feature"><div class="role-feature-check" style="background:var(--green-300);">✓</div> Submit issue reports</div>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="cta-section">
    ...
    <h2 class="cta-title">Join Barangay Magsaysay's<br>Smarter Waste System</h2>
    <p class="cta-desc">Sign in to your portal or register as a resident to track your household's waste collection in real time.</p>
    <a href="/login" class="btn-cta-white">Sign In to EcoTrack →</a>
  </section>

  <footer>
    <strong>EcoTrack</strong> · Smart Barangay Waste Collection System · Barangay Magsaysay, Zamboanga Sibugay &nbsp;·&nbsp; 2026
  </footer>
...
SIDEBAR_ADMIN = """<div class="sidebar" id="sidebar"><div class="sidebar-brand"><div class="sidebar-logo"><div class="sidebar-logo-icon">♻️</div><div class="sidebar-logo-text"><h1>EcoTrack</h1><span>Barangay Waste System</span></div></div></div><nav class="sidebar-nav"><div class="nav-section-label">Overview</div><a href="/admin" class="nav-link active"><span class="icon">🏠</span> Dashboard</a><a href="/admin/analytics" class="nav-link"><span class="icon">📊</span> Analytics & ML</a><div class="nav-section-label">Management</div><a href="/admin/zones" class="nav-link"><span class="icon">🗺️</span> Puroks</a><a href="/admin/schedules" class="nav-link"><span class="icon">📅</span> Schedules</a><a href="/admin/users" class="nav-link"><span class="icon">👥</span> Users</a><a href="/admin/reports" class="nav-link"><span class="icon">📋</span> Reports</a><div class="nav-section-label">Communications</div><a href="/admin/notifications" class="nav-link"><span class="icon">🔔</span> Notifications</a></nav><div class="sidebar-footer"><div class="sidebar-user"><div class="sidebar-user-avatar">{{ session.name[0] }}</div><div class="sidebar-user-info"><div class="sidebar-user-name">{{ session.name }}</div><div class="sidebar-user-role">{{ session.role }}</div></div><a href="/logout" title="Logout" style="color:var(--text-muted);font-size:16px;" onclick="return confirm('Are you sure you want to logout?')">⎋</a></div></div></div><div class="sidebar-overlay" id="overlay" onclick="closeSidebar()"></div>"""
SIDEBAR_COLLECTOR = """<div class="sidebar" id="sidebar"><div class="sidebar-brand"><div class="sidebar-logo"><div class="sidebar-logo-icon">♻️</div><div class="sidebar-logo-text"><h1>EcoTrack</h1><span>Collector Portal</span></div></div></div><nav class="sidebar-nav"><div class="nav-section-label">Collector</div><a href="/collector" class="nav-link active"><span class="icon">🚛</span> My Routes</a></nav><div class="sidebar-footer"><div class="sidebar-user"><div class="sidebar-user-avatar">{{ session.name[0] }}</div><div class="sidebar-user-info"><div class="sidebar-user-name">{{ session.name }}</div><div class="sidebar-user-role">Collector</div></div><a href="/logout" style="color:var(--text-muted);font-size:16px;" onclick="return confirm('Are you sure you want to logout?')">⎋</a></div></div></div><div class="sidebar-overlay" id="overlay" onclick="closeSidebar()"></div>"""
SIDEBAR_RESIDENT = """<div class="sidebar" id="sidebar"><div class="sidebar-brand"><div class="sidebar-logo"><div class="sidebar-logo-icon">♻️</div><div class="sidebar-logo-text"><h1>EcoTrack</h1><span>Resident Portal</span></div></div></div><nav class="sidebar-nav"><div class="nav-section-label">My Home</div><a href="/resident" class="nav-link active"><span class="icon">🏡</span> Dashboard</a></nav><div class="sidebar-footer"><div class="sidebar-user"><div class="sidebar-user-avatar">{{ session.name[0] }}</div><div class="sidebar-user-info"><div class="sidebar-user-name">{{ session.name }}</div><div class="sidebar-user-role">Resident</div></div><a href="/logout" style="color:var(--text-muted);font-size:16px;" onclick="return confirm('Are you sure you want to logout?')">⎋</a></div></div></div><div class="sidebar-overlay" id="overlay" onclick="closeSidebar()"></div>"""
